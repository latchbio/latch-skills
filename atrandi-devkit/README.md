...
This repo is for the Latch Agent to work specifically with the Atrandi RNA:DNA co-sequencing platform.
...
Contains Python library code with technology-specific helper functions the agent can import and use. Key modules:

- `common_const.py` — shared enums and constants (barcode positions, modality names, library types)
- `library_qc.py` — saturation curve computation and plotting with 10x reference datasets
...
3. Navigate to your tech docs directory and checkout your branch:
   ```bash
   cd /opt/latch/plots-faas/runtime/mount/agent_config/context/technology_docs/atrandi
   git fetch origin
